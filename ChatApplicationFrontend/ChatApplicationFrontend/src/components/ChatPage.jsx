...
const ChatPage = () => {
  ...
  useEffect(() => {
    ...
    client.connect({}, () => {
      ...
      client.subscribe(`/topic/room/${roomId}`, (message) => {
        ...
        setMessages((prev) => {
          if (!prev.some((msg) => msg.content === newMessage.content && msg.sender === newMessage.sender)) {
  ...
  return (
    <div>
      {/* Header Section */}
      <header className="fixed w-full py-5 shadow flex justify-around items-center dark:bg-gray-900">
      ...
      <main ref={chatBoxRef} className="py-20 px-10 w-2/3 dark:bg-slate-600 mx-auto h-screen overflow-auto">
        {messages.map((message, index) => (
          <div key={index} className={`flex ${message.sender === currentUser ? "justify-end" : "justify-start"}`}>
            <div className={`my-2 p-2 max-w-xs rounded ${message.sender === currentUser ? "bg-green-800" : "bg-gray-800"}`}>
              <div className="flex flex-row gap-2">
                <img className="h-10 w-10" src="https://avatar.iran.liara.run/public/43" alt="Avatar" />
